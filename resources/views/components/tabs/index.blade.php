...
<div class="w-full">
    <div  class="flex gap-2 overflow-x-auto border-b border-outline dark:border-outline-dark" >
        @foreach($tabs as $title => $route)
            @php
                $selected = request()->getUri($route) == $route
            @endphp
            <a @class([
...
{{--        <button 'font-bold text-primary border-b-2 border-primary dark:border-primary-dark dark:text-primary-dark' : 'text-on-surface font-medium dark:text-on-surface-dark dark:hover:border-b-outline-dark-strong dark:hover:text-on-surface-dark-strong hover:border-b-2 hover:border-b-outline-strong hover:text-on-surface-strong'" --}}
    </div>
